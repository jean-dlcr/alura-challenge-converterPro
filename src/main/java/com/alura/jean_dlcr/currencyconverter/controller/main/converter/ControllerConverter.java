...
public class ControllerConverter implements ActionListener {
    ...
    private void OnClickConverter() {
        ...
        String inputMountText = null;
        double inputMount = -1;
        double result = -1;
        double conversionRate = getConversionRate(codeFrom, codeTo);
        if (conversionRate > 0) {
            inputMountText = this.pnlConverter.txtInputFrom.getText();
            if (!Helper.isNullOrEmpty(inputMountText)) {
                try {
                    inputMount = Double.parseDouble(inputMountText);
                    ...
                    Logger.getLogger(ControllerConverter.class.getName()).log(Level.SEVERE, "Invalid input for conversion: " + inputMountText, ex);
